...
\begin{flashcard}[\studyArea]{Considerations for Delta Hedging}
    \begin{itemize}
        \item Delta is only an approximation and is less accurate for larger chaanges in the stock price.
...
\begin{flashcard}[\studyArea]{Effect of Interest Rate Swaps on Cash Flow Risk}
    \begin{flushleft}
        Entering into a fixed-payer swap reduces cash flow risk because future floating-rate payments are converted to certain fixed payments. However, this converts the low-duration floating payments to hihg-duration fixed payments. As a result, interest rates now affect the market value of the liability.
    \end{flushleft}
\end{flashcard}
